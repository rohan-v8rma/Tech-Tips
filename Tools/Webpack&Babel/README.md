...
- [Building](#building)
  ...
  - [Webpack `devServer` for hot reloading](#webpack-devserver-for-hot-reloading)
...
This tells webpack to use the `babel-loader` to transpile any `.js` files in your project that are not in the `node_modules` directory. The `@babel/preset-env` preset is used to configure Babel to transpile your code to a format that ensures maximum compatibility with older browsers that still have a high user base.
